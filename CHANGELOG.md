...
- Aligned remote autocompaction orchestration with Codex follow-up semantics: completed final answers no longer trigger redundant compaction, partial provider-overflow progress remains mid-turn, compact requests reuse the exact effective System and tool schemas, steering waits through the first post-compact continuation, and oversized tool outputs are bounded only at the transport boundary. Remote streams use a long cancellable operation deadline and bounded retries.
- Closed the remaining remote-compaction recovery edges: newly completed tool output participates in the preflight limit, rejected follow-up requests retain the original active-turn boundary, pending steering stays out of the mandatory continuation, pre-turn replay preserves media, active request controls remain frozen through compaction, Stop cancels the HTTP operation through response-body processing, unsupported compact output artifacts are discarded, and opaque state is checked against its base API model and backend snapshot. Manual `/compact` now honors the selected model, and the UI reports completion only after success.
- Persisted encrypted compaction now follows Codex across ChatGPT login changes: the new account receives one full request with the durable session state, while account-scoped response, reasoning, WebSocket, catalog, and retry state is never reused.
- Made remote autocompaction media-aware and aligned its effective window with Codex's current 272k ChatGPT Responses context: inline images use model-visible token estimates instead of base64 text size, oversized compact requests bound older tool outputs across the request while preserving image media from the complete active parallel-tool batch, and failed compact attempts no longer render a successful completion divider.
- Fixed ChatGPT OAuth tool-result preflight to recognize persisted OpenCode image `FilePart` attachments before Responses lowering, preventing large native-vision data URLs from being counted as text and triggering false mid-turn compaction.
- Fixed Web clipboard, drag-and-drop, and file-picker attachments on ordinary LAN HTTP origins by using a random browser-draft blob ID when WebCrypto hashing is unavailable; asynchronous paste failures now use the existing error toast instead of failing silently.
- Completed the Codex turn-routing contract for ChatGPT OAuth: same-turn sampling and remote compact retain sticky state from stream metadata, HTTP response headers, and WebSocket upgrade headers when the runtime exposes them across synthetic markers; a missing previous response performs one invisible full-request resync before model output; account affinity falls back safely to the OAuth subject when an account id is unavailable; and compact payload bounding preserves images from the complete active parallel-tool batch while rejecting requests that still cannot fit before network I/O.
