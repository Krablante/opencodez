...
- Made remote autocompaction media-aware and aligned its effective window with Codex's current 272k ChatGPT Responses context: inline images use model-visible token estimates instead of base64 text size, oversized compact requests bound older tool outputs across the request while preserving the newest tool image, and failed compact attempts no longer render a successful completion divider.
- Completed the Codex turn-routing contract for ChatGPT OAuth: same-turn sampling and remote compact replay the server's sticky turn state, a missing previous response performs one invisible full-request resync on a fresh socket, account affinity falls back safely to the OAuth subject when an account id is unavailable, and compact payload bounding preserves images from the complete active parallel-tool batch while rejecting requests that still cannot fit before network I/O.
