...
type OpenCodezMetadata = Record<string, unknown>
type CompatibleSessionInfo = SessionInfo & { metadata?: OpenCodezMetadata }
type CompatibleSessionCreateInput = Parameters<SessionApi["create"]>[0] & { metadata?: OpenCodezMetadata }
type CompatibleSessionApi = Omit<
  SessionApi,
  "create" | "prompt" | "command" | "shell" | "compact" | "rename" | "archive" | "remove"
> & {
  create: (input?: CompatibleSessionCreateInput) => Promise<CompatibleSessionInfo>
...
function sessionInfo(session: Session): CompatibleSessionInfo {
  return {
    id: session.id,
