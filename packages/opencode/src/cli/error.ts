...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, ${OpenCodezIdentity.productName} does not support MCP authentication yet.`
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`${OpenCodezIdentity.cliName} auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
