...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: `fetch and checkout a GitHub PR branch, then run ${OpenCodezIdentity.productName}`,
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found ${OpenCodezIdentity.productName} session: ${sessionUrl}`)
          UI.println(`Importing session...`)

          const importResult = yield* Effect.promise(() =>
            Process.text([OpenCodezIdentity.cliName, "import", sessionUrl], { nothrow: true }),
          )
    ...
    UI.println()
    UI.println(`Starting ${OpenCodezIdentity.productName}...`)
    UI.println()

    const opencodeArgs = sessionId ? ["-s", sessionId] : []
    const code = yield* Effect.promise(
      () =>
        Process.spawn([OpenCodezIdentity.cliName, ...opencodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`${OpenCodezIdentity.cliName} exited with code ${code}`))
  }),
})
