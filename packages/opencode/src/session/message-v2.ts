    ...
    if (msg.info.role === "user") {
      ...
      for (const part of msg.parts) {
        ...
        if (part.type === "compaction") {
          userMessage.parts.push({
            type: "text",
            text: part.remote ? CodexResponsesCompaction.CONTINUE_MARKER : "What did we do so far?",
          })
        }
...
export function filterCompacted(msgs: Iterable<WithParts>) {
  ...
  const turnIndex = turnID ? result.findIndex((msg) => msg.info.id === turnID) : -1
  if (
      ...
      .filter(
        (message, index) =>
          !(
            ...
            compactionIndex + index > turnIndex &&
