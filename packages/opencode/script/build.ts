...
const artifactName = process.env.OPENCODEZ_BUILD === "1" ? "opencodez" : pkg.name
const executableName = artifactName === "opencodez" ? "opencodez" : "opencode"
if (!skipInstall) {
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      target: name.replace(artifactName, "bun") as any,
      outfile: `dist/${name}/bin/${executableName}`,
      execArgv: [`--user-agent=opencode/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    files: {
      [treeSitterWorkerPath]: treeSitterWorker,
      ...(embeddedFileMap ? { "opencode-web-ui.gen.ts": embeddedFileMap.source } : {}),
    },
    entrypoints: [
      "./src/index.ts",
      workerPath,
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/${executableName}`
